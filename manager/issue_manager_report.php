...
// ดึงข้อมูลสินค้าที่เกี่ยวข้องกับ store ที่กำหนด
$orderQuery = "SELECT issue_id, order_id, issue_type, issue_description, resolution_type, resolution_info, 
report_date, resolution_date
               FROM issue_orders
               WHERE store_id = ?";
$orderStmt = mysqli_prepare($conn, $orderQuery);
mysqli_stmt_bind_param($orderStmt, "i", $store_id); // ใช้ store_id จาก session
...
// ปิดการเชื่อมต่อหลังจากดึงข้อมูลเรียบร้อยแล้ว
mysqli_close($conn);
?>
   ...
   <div class="container table-container">
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    <?php foreach ($orders as $order): ?> 
                        <tr>					
                            <td><?= htmlspecialchars($order['issue_id']) ?></td>
                            <td><?= htmlspecialchars($order['order_id']) ?></td>
                            <td><?= htmlspecialchars($order['issue_type']) ?></td>
                            <td><?= htmlspecialchars($order['issue_description']) ?></td>
                            <td><?= htmlspecialchars($order['resolution_type']) ?></td>
                            <td><?= htmlspecialchars($order['resolution_info']) ?></td>
                            <td><?= htmlspecialchars($order['report_date']) ?></td>
                            <td><?= htmlspecialchars($order['resolution_date']) ?></td>
    ...
    <script>
        ...
        function exportToCSV() {
            ...
            const headers = ["Issue ID", "Order ID", "Issue Type", "Issue Description", "Resolution Type","Resolution Info",
                "Report Date","Resolution Date"];
