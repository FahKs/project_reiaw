...
$orderQuery = "SELECT product_id, listproduct_id, store_id, expiration_date, status, quantity, location, manufacture_date,
updated_at, detail_order_id
               FROM product
               WHERE store_id = ?";
                ...
                <?php if (!empty($orders)): ?>
                    <?php foreach ($orders as $order): ?>
                        ...
                        <td><?= htmlspecialchars($order['product_id']) ?></td>
                            ...
                            <td><?= htmlspecialchars($order['manufacture_date']) ?></td>
                            <td><?= htmlspecialchars($order['updated_at']) ?></td>
                            <td><?= htmlspecialchars($order['detail_order_id']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="5" class="text-center">No orders available for this store.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
<script>
    ...
    function exportToCSV() {
        ...
        const headers = ["product_id", "listproduct_id", "store_id", "expiration_date", "status", "quantity", 
            "location", "manufacture_date","updated_at", "detail_order_id"];
